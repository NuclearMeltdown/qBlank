...
json::Value WriteProfile(const Profile& p) {
  ...
  img["nativeSnap"] = p.image.nativeSnap;
...
Profile ReadProfile(const json::Value& v) {
  ...
  p.image.nativeSnap = i["nativeSnap"].AsBool(true);
...
bool Config::Load(std::string* error) {
  ...
  app.matchResolution = a["matchResolution"].AsBool(true);
...
std::string Config::Serialize() const {
  ...
  a["vsync"] = app.vsync;
  a["vulkan"] = app.vulkan;
  a["matchResolution"] = app.matchResolution;
